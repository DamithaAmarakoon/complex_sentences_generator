...
def gen_eda(train_orig, output_file, alpha_sr, alpha_ri, alpha_rs, alpha_rd, num_aug=9):
  ...
  sinhala_sentence = codecs.open(train_orig, 'r', encoding='UTF-8').readlines()[0]

  # translate sinhala text
  translated_sentence = translate(sinhala_sentence)

  if translated_sentence:
    aug_sentences = eda(translated_sentence, alpha_sr=alpha_sr, alpha_ri=alpha_ri, alpha_rs=alpha_rs, p_rd=alpha_rd, num_aug=num_aug-1)
